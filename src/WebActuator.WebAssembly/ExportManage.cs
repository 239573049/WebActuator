...
public class ExportManage
{
    ...
    {
        ActuatorCompile.RunSubmission(code, async (diagnostic) =>
        {
            await WebActuatorApp.GetRequiredService<IJSRuntime>().InvokeVoidAsync("OnDiagnostic", diagnostic);
        }, async (exception) =>
        {
            await WebActuatorApp.GetRequiredService<IJSRuntime>().InvokeVoidAsync("OnException", exception);
